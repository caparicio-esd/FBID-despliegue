...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Setear el loglevel a warn para limitar nivel de logging - (que si no no se ve nada...)
    spark.sparkContext.setLogLevel("WARN")


    // trabajos para inferencia
    //Load the arrival delay bucketizer
    val base_path= "/Users/apabook/Desktop/fbid/01_practica_fbid_standalone"
    val arrivalBucketizerPath = "%s/models/arrival_bucketizer_2.0.bin".format(base_path)
    print(arrivalBucketizerPath)
    ...
    // kafka retorna los valores con una estructura fija, donde el campo value es un binario
    // lo casteamos a string para que convierta el binario a un json stringificado
    ...
    // hacemos una conversion de string a json con el struct de arriba anidado
    ...
    // extraemos el objeto json a columnas
    ...
    // lo mismo, extraemos el objeto json a columnas
    ...
    // creamos para este df una columna route tb
    ...
    // convierte df a vectores para sistema de inferencia
    ...
    // descarta algunas columnas para el modelo
    ...
    // Make the prediction
    // predice y dropea una columna
    val predictions = inferenceModel.transform(finalVectorizedFeatures)
      .drop("Features_vec")

    // Drop the features vector and prediction metadata to give the original fields
    // dropeamos alguans columnas más....
    ...
    // pasa por consola también para testear que todo va ok...
