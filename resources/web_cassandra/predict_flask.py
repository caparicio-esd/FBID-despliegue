...
mongo_docker_url = "mongodb://{}:{}".format(mongo_host, mongo_port)
mongo_client = mongo_docker_url if is_dockerized_env else "mongodb://localhost:27017"
...
cassandraHost = \
  ...
  if is_dockerized_env else "127.0.0.1"
cassandraPort = \
  os.environ.get("CASSANDRA_PORT") \
  if is_dockerized_env else "9042"
cassandraKeySpace = \
  os.environ.get("CASSANDRA_KEYSPACE") \
  if is_dockerized_env else "agile_data_science"
cassandraTable = \
  os.environ.get("CASSANDRA_TABLE") \
  if is_dockerized_env else "flight_delay_classification_response"
cassandraCluster = Cluster(
  port=cassandraPort, 
  contact_points=[cassandraHost]
)
cassandraClient = cassandraCluster.connect(cassandraKeySpace)
cassandraClient.row_factory = dict_factory
...
kafka_bootstrap_servers = [
    kafka_url] if is_dockerized_env else ['localhost:9092']
...
def classify_flight_delays_realtime_response(unique_id):
  ...
  #   }
  # )
  prediction = cassandraClient.execute("""
    SELECT * FROM {}.{}
    WHERE uuid = {}
    LIMIT 1
  """.format(
  ...
  if prediction.one() is not None:
    response["status"] = "OK"
    response["prediction"] = prediction.one()
    response["prediction"]["uuid"] = str(response["prediction"]["uuid"])
