...
RESPONSE_TOPIC = 'flight_delay_classification_response'
...
def search_flights():
    ...
    if tail_number:
        query['query']['bool']['must'].append(
            {'match': {'TailNum': tail_number}})
    if flight_number:
        query['query']['bool']['must'].append(
            {'match': {'FlightNum': flight_number}})
...
def classify_flight_delays_realtime():
    ...
    kafkaProducer.send(PREDICTION_TOPIC, message_bytes)
